...
private:
  ...
  long prevTime;                      // Previous time for RPM calculation
  long prevCount;                     // Previous encoder count for RPM calculation
  const int countsPerRotation;        // Encoder counts per full rotation
  const long millisToMinutes = 60000; // Milliseconds to minutes for RPM calculation
  float filteredRPM = 0;                    // Filtered RPM value
  float alpha;
  float rpmConstant = 0;
